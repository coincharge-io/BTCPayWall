...
registerBlockType('btcpaywall/gutenberg-pay-per-view-templates', {
	title: 'BTCPW Pay Per View Templates',
	icon: (
		<svg
			version="1.0"
...
-300 0z"
	...
	category: 'widgets',
	keywords: ['paywall', 'pay-per-view-templates'],
	attributes: {
		className: {
			type: 'string',
			default: '',
	...
	edit: (props) => {
		...
		const shortcodes = useSelect((select) =>
			select('btcpaywall/shortcode_data').getPayPerViewTemplateList()
		);
							...
							<PanelRow>
								<div className="btcpw_gutenberg_sel_num_control">
									<SelectControl
										...
										options={Object.entries(shortcodes).map(
											(pair) => ({
												label: pair[0],
												value: pair[1],
											})
		...
		return [
			<div>
				<ServerSideRender
					block="btcpaywall/gutenberg-pay-per-view-templates"
					attributes={{
						shortcode,
						className,
						title,
