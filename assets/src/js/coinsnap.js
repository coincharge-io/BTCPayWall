(function ($) {
	...
	$(document).ready(function () {
		...
		$('#btcpw_digital_download_form').submit(function (e) {
			...
			if (btcpw_invoice_id && form_container) {
				btcpwShowCoinsnapFileInvoice(btcpw_invoice_id, form_container);
				return;
			}
		...
		if (getCookie('btcpw_donation_initiated_' + payment.post_id)) {
			$.ajax({
				...
				data: {
					...
					invoice_id: getCookie(
						'btcpw_donation_initiated_' + payment.post_id
					),
					nonce_ajax: payment.security,
				},
				success: function (response) {
	...
	$(document).ready(function () {
		...
		$('#tipping_form_box').submit(function (e) {
			e.preventDefault();
			if (btcpw_invoice_id && form_container) {
				btcpwShowCoinsnapInvoice(btcpw_invoice_id, form_container);
				return;
			}
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.success) {
						...
						btcpwShowTippingCoinsnapInvoice(
							btcpw_invoice_id,
							form_container,
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.success) {
						btcpw_invoice_id = response.data.invoice_id;
						form_container = $(
							'.btcpw_widget.btcpw_tipping_box_container'
						);
						btcpwShowTippingCoinsnapInvoice(
							btcpw_invoice_id,
							form_container,
		...
		$('#skyscraper_tipping_high_form').submit(function (e) {
			...
			$.ajax({
				...
				data: {
					...
					email: $(
						'#btcpw_skyscraper_tipping_donor_email_high'
					).val(),
					address: $(
						'#btcpw_skyscraper_tipping_donor_address_high'
					).val(),
					phone: $(
						'#btcpw_skyscraper_tipping_donor_phone_high'
					).val(),
					message: $(
						'#btcpw_skyscraper_tipping_donor_message_high'
					).val(),
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.success) {
						btcpw_invoice_id = response.data.invoice_id;
						form_container = $(
							'.btcpw_widget.btcpw_skyscraper_banner.high'
						);
						btcpwShowTippingCoinsnapInvoice(
							btcpw_invoice_id,
							form_container,
		...
		$('#skyscraper_tipping_wide_form').submit(function (e) {
			...
			$.ajax({
				...
				data: {
					...
					email: $(
						'#btcpw_skyscraper_tipping_donor_email_wide'
					).val(),
					address: $(
						'#btcpw_skyscraper_tipping_donor_address_wide'
					).val(),
					phone: $(
						'#btcpw_skyscraper_tipping_donor_phone_wide'
					).val(),
					message: $(
						'#btcpw_skyscraper_tipping_donor_message_wide'
					).val(),
	...
	function btcpwShowCoinsnapFileInvoice(
		invoice_id,
		form_container,
		btcpw_order_id,
		amount
	...
	function btcpwShowCoinsnapInvoice(
		invoice_id,
		form_container,
		order_id,
		amount,
	...
	function btcpwShowTippingCoinsnapInvoice(
		invoice_id,
		form_container,
		redirectTo = null
	) {
