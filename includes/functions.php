...
function get_download_url($payment_id, $file, $download_id, $email)
{
    ...
    $payment = new BTCPayWall_Payment($params['key']);
    ...
    if (!empty($payment->id)) {
        ...
        $args = array(
            'btcpw_file' => rawurlencode(sprintf('%d:%d:%d', $payment->id, $params['download_id'], $params['digital_file']))
        );
        ...
        $args['digital_file']  = $params['digital_file'];
        $args['token'] = bin2hex(random_bytes(14));
    }
